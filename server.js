...
['prompt_id TEXT', 'prompt_text TEXT', 'rating INTEGER', 'diarize INTEGER DEFAULT 0', 'result_clean TEXT', 'duration_sec REAL', 'video_limit_mb INTEGER DEFAULT 200'].forEach(col => {
...
const ALL_ALLOWED = new Set([...AUDIO_EXTS, ...VIDEO_EXTS]);
...
// Хелпер: получить чистый транскрипт (result_clean → fallback result_srt)
...
app.get('/api/jobs/:id/download/docx', authMiddleware, async (req, res) => {
  ...
  if (!job.result_txt && !job.result_clean && !job.result_srt) {
    return res.status(404).json({ error: 'Результат не готов' });
  }

  // Саммари + чистая транскрипция (fallback на result_srt)
  const summary = job.result_txt ? job.result_txt.split('\n---\n')[0].trim() : '';
  const transcript = getTranscript(job);
