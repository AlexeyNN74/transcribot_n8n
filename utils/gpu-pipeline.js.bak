...
async function shelveGpu() {
  try {
    ...
    const pending = db.prepare("SELECT COUNT(*) as cnt FROM jobs WHERE status='queued'").get();
    if (pending.cnt > 0) {
  ...
  try {
    db.prepare(`
      UPDATE gpu_sessions SET
        ...
        jobs_count=?, job_ids=?, status='closed'
      WHERE id=?
    `).run(processedInSession.length, JSON.stringify(processedInSession), currentSessionId);
...
async function startJob(job) {
  ...
  try {
    ...
    db.prepare("UPDATE jobs SET status='error', error=? WHERE id=?").run(e.message, job.id);
...
async function handleDone(jobId, payload) {
  ...
  try {
    ...
    db.prepare(`
      UPDATE jobs SET
        status='completed', result_txt=?, result_srt=?, result_json=?,
        result_clean=?, duration_sec=?, completed_at=datetime('now'), progress=100
      WHERE id=?
    ...
    db.prepare("UPDATE jobs SET status='error', error=? WHERE id=?")
      .run(`Post-processing: ${e.message}`, jobId);
...
async function handleError(jobId, payload) {
  ...
  if (job) {
    db.prepare("UPDATE jobs SET status='error', error=? WHERE id=?")
      .run(payload.message || 'GPU processing error', jobId);
...
function handleTimeout(jobId) {
  ...
  if (job && job.status === 'processing') {
    db.prepare("UPDATE jobs SET status='error', error=? WHERE id=?")
      .run('Timeout: GPU не ответил', jobId);
...
async function finishJob(jobId) {
  ...
  const next = db.prepare("SELECT id FROM jobs WHERE status='queued' ORDER BY created_at ASC LIMIT 1").get();
  if (next) {
...
function recoverOnStartup() {
  ...
  const stuck = db.prepare("SELECT id FROM jobs WHERE status='processing'").all();
  if (stuck.length > 0) {
    log(`Recovery: ${stuck.length} stuck jobs → queued`);
    db.prepare("UPDATE jobs SET status='queued', progress=0 WHERE status='processing'").run();
